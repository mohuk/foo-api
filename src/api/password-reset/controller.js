...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Foster Publications account.<br>
        ...
        &mdash; Foster Publications Team
      `
      return sendMail({ toEmail: email, subject: 'Foster Publications - Password Reset', content })
    })
